...
module.exports =
  ...
  activate: ->
    require('atom-package-deps').install('lint-sass-vue')
    @subs = new CompositeDisposable
    @subs.add atom.config.observe 'lint-sass-vue.noConfigDisable',
      (noConfigDisable) =>
        @noConfigDisable = noConfigDisable
    @subs.add atom.config.observe 'lint-sass-vue.configFile',
      (configFile) =>
        @configFile = configFile
    @subs.add atom.config.observe 'lint-sass-vue.globalSassLint',
      (globalSassLint) =>
        @globalSassLint = globalSassLint
    @subs.add atom.config.observe 'lint-sass-vue.globalNodePath',
      (globalNodePath) =>
        @globalPath = globalNodePath
    @subs.add atom.config.observe 'lint-sass-vue.resolvePathsRelativeToConfig',
      (resolvePathsRelativeToConfig) =>
  ...
  # Determines whether to use the sass-lint package included with lint-sass-vue
  ...
  provideLinter: ->
    provider =
      name: 'sass-lint'
      grammarScopes: ['source.vue', 'text.html.vue']
      scope: 'file'
      lintOnFly: true
      lint: (editor) =>
        ...
        try
          ...
          if error.message is 'prefix' then atom.notifications.addError """
            **Error getting $PATH - lint-sass-vue**\n
            ...
            Please make sure Atom is getting $PATH correctly or set it directly in the `lint-sass-vue` settings.
          """, {dismissable: true}
          return []
          ...
          atom.notifications.addWarning """
            **Sass-lint package missing**
            ...
            You can always use the sass-lint pacakage included with lint-sass-vue by disabling the
        ...
        try
          ...
          if match
            ...
            console.log('lint-sass-vue', error)
            return [
              type: 'Error'
